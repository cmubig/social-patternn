...
def plot_trajectories_mp(
    config, hist, fut, pred, seq_start_end, best_sample_idx, fut_mp, pred_mp, 
    filename='val_'):
    """ Trajectory plot wrapper.
    Inputs:
    -------
    ...
    filename[str]: filename
    """
    assert os.path.exists(config.motion_primitives), \
        f"File: {config.motion_primitives} does not exist"
        
    f = open(config.motion_primitives, "r")
    mp_classes = f.readlines()
    
    for i, (s, e) in enumerate(seq_start_end):
        idx = best_sample_idx[s:e]
        fn = filename+ f"_seq-{i}"
        h, f, p_multi = hist[:, :, s:e], fut[:, :, s:e], pred[:, :, :, s:e]
        fmp, pmp = fut_mp[:, s:e], pred_mp[:, s:e]
        ...
        plot_2d_trajectories_mp(
            config=config, hist=h, fut=f, pred=p, fmp=fmp, pmp=pmp, classes=mp_classes, 
            max_agents=n, filename=fn)
    ...
    if os.path.exists(config.background):
        ...
        plt.imshow(background, zorder=0,
            extent=[config.x_lim[0], config.x_lim[1] - config.diff,
                    config.x_lim[0], config.y_lim[1]], alpha=0.6)
...
def plot_2d_trajectories_mp(
    config, hist, fut, pred, fmp, pmp, classes, max_agents = 100, filename='val_'):
    """ Plots full trajectories from trajair dataset, i.e. hist + ground truth 
    and predictions.
    Inputs
    ------
    ...
    for agent in range(num_agents):
        ...
        if agent == 0:
            ...
            fmp_, pmp_ = fmp[-1, agent].cpu().item(), pmp[-1, agent].cpu().item()
            fmp_, pmp_ = classes[fmp_].strip("\n"), classes[pmp_].strip("\n")
            mp_str = f"fut:{fmp_} pred:{pmp_}"
            ax.annotate(mp_str, xy=(x_mp, y_mp), fontsize=8, color='C'+str(agent % 10))
            y_mp += 0.5
    ...
    if os.path.exists(config.background):
        ...
        plt.imshow(background, zorder=0,
            extent=[config.x_lim[0], config.x_lim[1] - config.diff,
                    config.x_lim[0], config.y_lim[1]], alpha=1.0)
